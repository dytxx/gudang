...
const chartConfig = {
  ...
  pass: {
    label: "QC Pass",
    color: "var(--chart-1)", // Warna Biru
  },
  remaining: {
    label: "Masih di WO",
    color: "var(--muted-foreground)", // Warna Abu-abu
  },
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(json)) {
          // Filter untuk SYSTEM 1
          const systemData = json.filter((item: any) => item.part_number === "SYSTEM 1")
        ...
        console.error("Gagal mengambil data SYSTEM 1:", error)
      }
    }

    fetchData()
    // Refresh setiap 5 detik (opsional)
  ...
  return (
    <Card className="flex flex-col h-full">
      <CardHeader className="items-center pb-0">
        <CardTitle>Stok SYSTEM 1</CardTitle>
