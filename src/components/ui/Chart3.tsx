...
const chartConfig = {
  ...
  pass: {
    label: "QC Pass",
    color: "var(--chart-2)", // Warna Hijau/Teal
  },
  remaining: {
    label: "Masih di WO",
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(json)) {
          // Filter untuk SYSTEM 2
          const systemData = json.filter((item: any) => item.part_number === "SYSTEM 2")
        ...
        console.error("Gagal mengambil data SYSTEM 2:", error)
      }
    }
  ...
  return (
    <Card className="flex flex-col h-full">
      <CardHeader className="items-center pb-0">
        <CardTitle>Stok SYSTEM 2</CardTitle>
